...
\texttt{done} handler is usually added to a request. Under Oboe each
separately addressed area of interest inside the JSON resource requires
its own handler so it is helpful to provide streamlined support for
adding several selector-handler pairs at a time. A shortcut style is
provided:
...
common using partial completion. Because \texttt{'!'} is the JSONPath
for the root of the document, for some callback \texttt{c},
...
principled design. Because they are not REST these streaming frameworks
are not suitable even as a fallback when running on legacy platforms and
will not be used.

Although Oboe is not being designed for live events, it is interesting
to speculate whether it could be used as a REST-compatible bridge to
unify live-ongoing feeds with ordinary REST resources. Consider a REST
service which gives results per-constituency for UK general elections.
When requesting historic results the data is delivered in JSON format
much as usual. Requesting the results for the current year on the night
of the election, an incomplete JSON with the constituencies known so far
