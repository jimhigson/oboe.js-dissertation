...
\emph{extra challenges}, Code which would be normally thought of as
compact is not necessarily so after gzipping, for example repetition of
logic compresses very well, often better than if a generalised form is
extracted. Contrast with DRY.
...
the distributable form. This service is provided gratis to open source
projects. When a third-party contributes a bug fix the changes are
automatically annotated with their test status so that no pull request
which breaks the build may be accepted. This automation encourages
collaboration by making it very cheap to accept third party patches.
