...
One benefit of a unified model for streamed and short-lived content is
that it allows a simpler security model. Because the transport is
different, streaming security is usually implemented separately from
other HTTP requests. Schneier argues that ``complexity is the worst
enemy of security'' (Schneier 2000 Software Complexity and Security) and
in one online debate paints a buildings analogy (Schneier 2013):
...
a building having only one entrance and a better level of security
should be possible given the same resources.
...
---------. 2013. ``To Profile or Not to Profile?''
